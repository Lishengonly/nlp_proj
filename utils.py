...
def dataProcessing(samples):
  question_ids = []
  sentences = []
  labels = []
  for q_id, sample in samples.items():
    sentence = sample[0].strip().split(' ')
    last = sentence.pop()
    sentence.append(last[:-1])
    sentence.append(last[-1])
    pronoun = sample[1].strip().split(' ')
    candidates = sample[2].strip().split(',')
    correct = sample[3].strip()
    idx = sentence.index(pronoun[0])
    for p in pronoun:
      sentence.pop(idx)
    if candidates[0].lower() == correct.lower():
      st = sentence.copy()
      st.insert(idx, candidates[0])
      sentences.append(' '.join(st).split(' '))
      labels.append(True)
      question_ids.append(q_id)

      st = sentence.copy()
      st.insert(idx, candidates[1])
      sentences.append(' '.join(st).split(' '))
      labels.append(False)
      question_ids.append(q_id)
    elif candidates[1].lower() == correct.lower():
      st = sentence.copy()
      st.insert(idx, candidates[1])
      sentences.append(' '.join(st).split(' '))
      labels.append(True)
...
def getTest1():
    f_dir = os.path.join('Winograd/test1/test1.c.txt')
    with open(f_dir, 'r') as f:
      ...
      for line in lines:
        if count not in samples:
          samples[count] = []
        samples[count].append(line)
        if line == '\n':
          count += 1

      return dataProcessing(samples)


def getTest2():
  tree = ET.parse('Winograd/test2/WSCollection.xml')
  root = tree.getroot()
  samples = {}
  count = 0
  for child in root:
    samples[count] = []
    samples[count].append(child[0][0].text.strip() + ' ' + child[0][1].text.strip() + ' ' + child[0][2].text.strip())
    samples[count].append(child[0][1].text.strip())
    samples[count].append(child[2][0].text.strip() + ',' + child[2][1].text.strip())
    if child[3].text.strip()[0] == 'A':
      samples[count].append(child[2][0].text.strip())
    elif child[3].text.strip()[0] == 'B':
      samples[count].append(child[2][1].text.strip())
    count += 1
